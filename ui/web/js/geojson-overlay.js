L.Control.GeoJSONOverlay = L.Control.extend({
    ...
    onAdd: function (map) {
        ...
        L.DomEvent.on(self._toggleAutoRefreshButton, 'click',
            self._toggleAutoRefresh, self);
        ...
        L.DomEvent.on(self._toggleAreasButton, 'click',
            self._toggleAreas, self);
        ...
        L.DomEvent.on(self._toggleVisibilityGraphButton, 'click',
            self._toggleVisibilityGraphs, self);
    ...
    _requestData: function () {
        ...
        if (self._map.getZoom() < 17) {
            if (self._autoRefresh ||
                !window.confirm("Wirklich Daten für Zoomstufe "
                    + self._map.getZoom() + " laden?")) {
                return;
            }
        }
        ...
        var request = {
            waypoints: [bounds.getWest(), bounds.getNorth(), bounds.getEast(), bounds.getSouth()],
        ...
        layer.bindPopup(function (l) {
            ...
            for (var key in feature.properties) {
                ...
                if (key == 'osm_way_id' && value != 0) {
                    ...
                    osmLink.href =
                        'https://www.openstreetmap.org/way/' + Math.abs(value);
