L.Control.MapLinks = L.Control.extend({
    ...
    onAdd: function (map) {
        ...
        self._container.appendChild(self._createButton(
            'O', 'OpenStreetMap', function () {
                var center = this._map.getCenter();
                var zoom = Math.round(this._map.getZoom());
                var url = 'https://www.openstreetmap.org/#map=' +
                    zoom + '/' +
        ...
        self._container.appendChild(self._createButton(
            'G', 'Google Maps', function () {
                var center = this._map.getCenter();
                var zoom = Math.round(this._map.getZoom());
                var url = 'https://maps.google.de/maps' +
        ...
        self._container.appendChild(self._createButton(
            'MC', 'Map Compare', function () {
                var center = this._map.getCenter();
                var zoom = Math.round(this._map.getZoom());
                var url = 'http://tools.geofabrik.de/mc/#' +
                    zoom + '/' +
        ...
        self._container.appendChild(self._createButton(
            'OT', 'Overpass Turbo', function () {
                var center = this._map.getCenter();
                var zoom = Math.round(this._map.getZoom());
                var query = window.prompt('Overpass Turbo Anfrage (Wizard-Syntax, z.B. highway=primary oder highway=*):', '');
                if (query === null) {
                    return;
                }
                var url = 'http://overpass-turbo.eu/?C=' +
                    encodeURIComponent(
        ...
        self._container.appendChild(self._createButton(
            'Ex', 'Osmium Extract', function () {
                var bounds = this._map.getBounds();
                var b = '-b ' + bounds.getWest().toFixed(6) + ',' +
            ...
            'J', 'JOSM', function () {
                ...
                window.fetch(url).then(
                    function (response) {
                        ...
                        alert('Verbindung zu JOSM fehlgeschlagen (' + reason +
                            '). JOSM muss laufen und die Fernsteuerung aktiviert sein.');
                        window.open('https://josm.openstreetmap.de/wiki/Help/Preferences/RemoteControl');
                    });
            }));
