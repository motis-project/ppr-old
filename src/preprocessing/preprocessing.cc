...
private:
  ...
  void set_missing_street_nodes(std::vector<oriented_int_edge>& sorted_edges) {
    for_edge_pairs_ccw(
  ...
  std::vector<oriented_int_edge> edges_sorted_by_angle(int_node* in) {
    std::vector<oriented_int_edge> sorted_edges;
  ...
  crossing_type::crossing_type combine_crossing(
      crossing_type::crossing_type a, crossing_type::crossing_type b) {
    return std::max(a, b);
  }

  bool is_reverse(double ref_angle, double other_angle) {
    auto angle = normalized_angle(ref_angle - other_angle);
    return is_reverse(angle);
  }

  bool is_reverse(double angle) { return angle > PI / 2 && angle < PI * 3 / 2; }
